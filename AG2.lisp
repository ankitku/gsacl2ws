...
(defconst *AttestationSubnetCount* 6)         ;; ARBITRARY 
...
(defconst *eth-twp*
  `((AGG . (,*eth-default-agg-weights* . ,*eth-default-aggregate-params*))
    ...
    (SUB5 . (,*eth-default-agg-subnet-weights*
             . ,*eth-default-aggregate-subnet-params*))
    (SUB6 . (,*eth-default-agg-subnet-weights* . ,*eth-default-aggregate-subnet-params*))))
...
(definecd emit-evnts (p1 :peer p2 :peer ts ats :lot n elapsed :nat)
  :loev
  ...
  (app (emit-meshmsgdeliveries-peer-topics p1 p2 (set-difference-equal ts ats) n)
       ;(emit-meshmsgdeliveries-peer-topics p1 p2 ats 1)
       `((,p2 HBM ,elapsed)))))


(skip-proofs
;; emit attack events from p1 to p2, in the attacked topic top
(definecd emit-evnts-ticks (p1 :peer p2 :peer ts ats :lot n elapsed
                               ticks :nat)
  :loev
  (app (emit-evnts p1 p2 ts ats n elapsed)
       (if (<= (- ticks elapsed) 0)
           nil
         (emit-evnts-ticks p1 p2 ts ats n elapsed (- ticks elapsed))))))


(emit-evnts-ticks 'A 'V (topics) '(SUB1 SUB2) 3 20 100)
...
;; attacker peer p
(definec scorePropViolation-ag2 (ps :peer-state p :peer top1 top2 :topic) :boolean
  :skip-tests t
  ...
  (^ (> (lookup-score p (calc-nbr-scores-map (peer-state-nbr-tctrs ps)
                                              ...
                                              *eth-twp*))
         0)
      (< (calcScoreTopic (lookup-tctrs p top1 (peer-state-nbr-tctrs ps))
                          (mget top1 *eth-twp*))
         0)
      (< (calcScoreTopic (lookup-tctrs p top2 (peer-state-nbr-tctrs ps))
                          (mget top2 *eth-twp*))
         0)))
...
(skip-proofs
(definecd eth-attack-violations (gr :group evnts :loev p1 :peer p2 :peer top1 top2 :topic i :nat twpm :twp s :nat acc :lob) :lob
      ...
      (b* (((mv k s) (defdata::genrandom-seed
		...
		next-actor-events
	       ...
	       top1
               ...
               s
               (if (hbm-evntp ev)
                   (cons (scorePropViolation-ag2 (mget p2 gr) p1 top1 top2) acc)
                 acc)))))
)
...
(time$
 (b* ((ticks 200)
      ...
      (grp (initialize-group-of-meshpeers '(A B)
                                          '(A B)
                                          '(AGG BLOCKS SUB1 SUB2 SUB3 SUB4 SUB5 SUB6)
                                          100))
      (evnts (emit-evnts-ticks 'A 'B '(AGG BLOCKS SUB1 SUB2 SUB3 SUB4 SUB5
                                           SUB6)
                               `(,attacktopic1 ,attacktopic2) 20 18 ticks)))
    (eth-attack-violations grp evnts 'A 'B attacktopic1 attacktopic2 (* 10 (len evnts))
                           *eth-twp* 42 nil)))
...
;; 29.5 mins
;; ; 1771.55 seconds realtime, 1673.25 seconds runtime
...
(time$
 (b* ((ticks 100)
      ...
      (grp (reduce* graph->group 
                   ...
                   *goerli*
                   '(AGG BLOCKS SUB1 SUB2 SUB3 SUB4 SUB5 SUB6)))
      (evnts (emit-evnts-ticks 'P222 'P834 '(AGG BLOCKS SUB1 SUB2 SUB3 SUB4
                                                 SUB5 SUB6)
                               `(,attacktopic1 ,attacktopic2) 200 18 ticks)))
   ;; P222 is attacker, P834 is observer who records violations
   (eth-attack-violations grp evnts 'P222 'P834 attacktopic1 attacktopic2 (* 10 (len evnts))
                          *eth-twp* 42 nil)))
